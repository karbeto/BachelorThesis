...
export function useRoutingLogic(fixedMunicipalityId?: number) {
  ...
  const { data: routings, isLoading: routingsLoading } = useQuery({
    ...
    queryFn: () =>
      getRoutings(
        fixedMunicipalityId ? { municipality_id: fixedMunicipalityId } : {},
      ),
  });
  ...
  const handleRoutingSubmit = () => {
    ...
    if (editingRouting) {
      ...
      const finalMunicipalityId =
        fixedMunicipalityId ||
        (routingForm.municipality_id ? Number(routingForm.municipality_id) : null);

      if (isSuperAdmin && !finalMunicipalityId) {
        toast.error("Изберете општина");
        return;
      }

      if (!routingForm.category_id) {
        toast.error("Пополнете го полето за категорија");
        return;
      }

      createMutation.mutate({
        municipality_id: finalMunicipalityId ? Number(finalMunicipalityId) : undefined,
...
export function useMunicipalityAdminLogic() {
  ...
  const profile = getStoredProfile();
  const municipalityId: number | undefined = profile?.municipality_id;

  const municipalityName: string | undefined =
    municipalities?.find((m: any) => m.id === municipalityId)?.name ||
